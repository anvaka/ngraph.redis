...
function createScriptRunner(scripts, client) {
  ...
  function forEachLinkedNode(nodeId, callback, kind) {
    kind = kind || '';
    var queue = [[scripts.forEachLinkedNode, 1, nodeId, kind, 0]]
    return handleNextInQueue();

    function handleNextInQueue() {
      var command = queue.pop()
      return run(command).then(parseResult).then(checkIfNeedMore);
    }
    ...
    function iterate(hash, kind) {
      ...
      links.forEach(function(link) {
        ...
        callback(args); // TODO: Do we need early cancellation? If so, it can be done here
      });

      var cursor = hash[kind + '.cursor'];
      if (cursor === undefined || cursor == '0') return;

      // schedule next chunk of edges:
      queue.push([scripts.forEachLinkedNode, 1, nodeId, kind, cursor]);
    }
  }
